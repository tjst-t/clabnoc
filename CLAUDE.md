...
- **フロントエンド実装**: フロントエンドのUI実装・変更時は必ず `frontend-design` スキルを使用すること。プロダクショングレードの高品質なデザインを生成するために活用する。
- テスト: テーブル駆動テスト、mockは interface で注入
- コミットメッセージ: Conventional Commits (feat:, fix:, docs:, test:, refactor:)

## コミット方針

- 実装はある程度細かい単位でコミットすること（1つの論理的な変更ごと）
- 例: 新しいパッケージ追加、APIエンドポイント追加、コンポーネント追加、テスト追加、バグ修正など、それぞれ別コミット
- 大きな変更を1つのコミットにまとめない。レビューしやすい粒度を意識する
- テストが通る状態でコミットすること（壊れた状態でコミットしない）
